    ...
    with open(file, 'r') as ha:  # 打开haproxy配置文件###
        for line in ha:
            ...
            # 匹配配置文件以server开头的行###
                ...
                backend_name_dict[backend_name].append(
                    server_dict)  # 将server字典与backend的名称进行关联###
            else:
...
def backend_server_show(backend_show_value, server_show_dict):
    ...
    print('%-5s  %-20s  %-20s  %-20s  %-20s' %
          ('id', 'name', 'address', 'weight', 'maxconn'))
...
if __name__ == '__main__':
    ...
    while flag:
        ...
        if select_num == '1':
            ...
            while query_flag:
                backend_index = raw_input(
                    '\033[32;1m请输入查询的backend编号或名称(返回上层菜单,请输入b)：\033[0m')

                ##############################对输入值进行判断################
                (backend_name, query_flag) = backend_input_if(
                    backend_index, haproxy_name_show_dict)
        ...
        if select_num == '2':
            ...
            while add_flag:
                backend_index = raw_input(
                    '\033[32;1m请输入添加的backend的编号或名称(返回上层菜单,请输入b)：\033[0m')
                ##############################对输入值进行判断################
                (backend_name, add_flag) = backend_input_if(
                    backend_index, haproxy_name_show_dict)
                ...
                if backend_name:  # 显示对应backend下的server列表###
                    ...
                    print(
                        '\033[34;1m请依次输入后端服务信息(name,address,weight,maxconn):\033[0m')
                    # 对输入的name有效性进行判断###
                    add_server_dict['name'] = name_decide()
                    # 对输入的IP有效性进行判断###
                    add_server_dict['address'] = ipaddress_decide()
                    add_server_dict['weight'] = number_decide(
                        '权重值')  # 对输入的权重有效性进行判断###
                    add_server_dict['maxconn'] = number_decide(
                        '最大连接数')  # 对输入的连接数有效性进行判断###

                    print(add_server_dict['name'], add_server_dict[
                          'address'], add_server_dict['weight'], add_server_dict['maxconn'])
                    ...
                    if add_server_dict['name'] and add_server_dict['address'] and add_server_dict['weight'] and add_server_dict['maxconn']:
                        ...
                        if add_commit == 'y':  # 确认添加服务条目，并回写配置文件###
                            haproxy_server_show_dict[
                                backend_name].append(add_server_dict)
                            backend_server_handle(
                                haproxy_file, haproxy_server_show_dict)
                        else:
        ...
        if select_num == '3':
            ...
            while backend_modify_flag:
                backend_index = raw_input(
                    '\033[32;1m请输入修改的backend的编号或名称(返回上层菜单,请输入b)：\033[0m')
                ##############################对输入值进行判断################
                (backend_name, backend_modify_flag) = backend_input_if(
                    backend_index, haproxy_name_show_dict)
                ...
                if backend_name:  # 显示对应backend下的server列表###
                    ...
                    while server_modify_flag:
                        ...
                        server_index = raw_input(
                            '\033[32;1m请输入修改的server的编号(返回上层菜单,请输入b)：\033[0m')
                        ...
                        #####################指定具体的server编号进行判断#####
                            ...
                            modify_server_dict[
                                'maxconn'] = number_decide('最大连接数')

                            print(modify_server_dict['name'], modify_server_dict[
                                  'address'], modify_server_dict['weight'], modify_server_dict['maxconn'])
                            ...
                            if modify_server_dict['name'] and modify_server_dict['address'] and modify_server_dict['weight'] and modify_server_dict['maxconn']:
                                ...
                                if modify_commit == 'y':  # 确认修改服务条目，并回写配置文件###
                                    haproxy_server_show_dict[backend_name][
                                        server_index - 1] = modify_server_dict
                                    backend_server_handle(
                                        haproxy_file, haproxy_server_show_dict)
        ...
        if select_num == '4':
            ...
            while backend_delete_flag:
                backend_index = raw_input(
                    '\033[32;1m请输入删除的backend下条目编号或名称(返回上层菜单,请输入b)：\033[0m')
                ##############################对输入值进行判断################
                (backend_name, backend_delete_flag) = backend_input_if(
                    backend_index, haproxy_name_show_dict)
                ...
                if backend_name:  # 显示对应backend下的server列表###
                    ...
                    while server_delete_flag:
                        ...
                        server_index = raw_input(
                            '\033[32;1m请输入删除的server的编号(返回上层菜单,请输入b)：\033[0m')
                        if len(server_index) != 0:
                            server_index = input_handle(server_index)
                        if server_index == 'b':  # 输入b，返还上一层###
                        ...
                        #####################指定具体的server编号进行判断#####
                            ...
                            print(haproxy_server_show_dict[
                                  backend_name][server_index - 1])
                            delete_commit = raw_input('请确认是否删除此条目（y | n）：')
                            if delete_commit == 'y':  # 确认删除服务条目，并回写配置文件###
                                del haproxy_server_show_dict[
                                    backend_name][server_index - 1]
                                backend_server_handle(
                                    haproxy_file, haproxy_server_show_dict)
